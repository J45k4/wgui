use crate::ast::{AttrValue, Element, Expr, Node};
...
use crate::compiler::registry::schema_for;
use crate::diagnostic::{Diagnostic, Span};
use std::collections::HashMap;

pub fn lower(
	doc: &crate::compiler::validate::ValidatedDocument,
	module_name: &str,
		...
		match &nodes[i] {
			...
			Node::Element(el) if el.name == "For" => {
				let each = get_expr_attr(el, "each")
					.unwrap_or(Expr::Literal(crate::ast::Literal::Null, el.span));
		...
		if attr.name == name {
			match &attr.value {
				...
				AttrValue::Number(value, span) => {
					return Some(Expr::Literal(crate::ast::Literal::Number(*value), *span));
				}
				_ => {}
			}
		}
