...
fn emit_custom(widget: &IrWidget) -> String {
	...
	for prop in &widget.props {
		match prop {
			IrProp::Literal { name: prop_name, value } if prop_name == "name" => {
				name = format!("{:?}", value)
			}
			IrProp::Literal { name: prop_name, value } if prop_name == "src" => {
				name = format!("{:?}", value)
			}
			IrProp::Value { name: prop_name, expr } if prop_name == "name" || prop_name == "src" => {
				name = emit_string_expr(expr)
			}
			IrProp::Literal { name: prop_name, value } if prop_name == "entry" => {
				entry = format!("{:?}", value)
			}
			IrProp::Value { name: prop_name, expr } if prop_name == "entry" => {
				entry = emit_string_expr(expr)
			}
			IrProp::Literal { name: prop_name, value } if prop_name == "props" => {
				props = format!("wgui::serde_json::json!({:?})", value)
			}
			IrProp::Number { name: prop_name, value } if prop_name == "props" => {
				props = format!("wgui::serde_json::json!({})", value)
			}
			IrProp::Bool { name: prop_name, value } if prop_name == "props" => {
				props = format!("wgui::serde_json::json!({})", value)
			}
			IrProp::Value { name: prop_name, expr } if prop_name == "props" => {
				props = format!("wgui::serde_json::json!({})", emit_expr(expr))
			}
			_ => {}
		}
	}
